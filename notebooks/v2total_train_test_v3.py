...
if(len(sys.argv)>1):
    ...
    DATA_ROOT_PATH = "/workspace/mnt/storage/zhangjunkang/gldv2/data/"
...
# 3-----------------------loss weight
train_csv = pd.read_csv(DATA_ROOT_PATH + 'train_labels/train.csv')
from collections import Counter
landmarkIdCounter = dict(Counter(train_csv['landmark_id']))
train_csv['counts'] = [landmarkIdCounter[x] for x in train_csv['landmark_id']]
countIdList = []
for key in sorted(landmarkIdCounter):
    countIdList.append(landmarkIdCounter[key])

s_mean = np.mean(1/(np.log(np.array(train_csv['counts']))))
scaleV = 1/ s_mean
# scaleV = 1/ np.mean(1/(np.log(np.array(train_csv['counts']))))
...
TRAIN_FILENAMES = []
TRAIN_LIST = list(train_csv['id'])
random.shuffle(TRAIN_LIST)
for i, img_name in enumerate(TRAIN_LIST):
    TRAIN_FILENAMES.append(TRAIN_GCS_PATH + img_name + ".tfrec")

val_16fold = pd.read_csv(DATA_ROOT_PATH + 'dataVal30_stage1.csv')
VALID_FILENAMES = []
VAL_LIST = list(val_16fold['images'])
random.shuffle(VAL_LIST)
for i, img_name in enumerate(VAL_LIST):
    VALID_FILENAMES.append(TRAIN_GCS_PATH + img_name + ".tfrec")
...
# 6-----------------------ArcMargin
class ArcMarginProduct_v21(tf.keras.layers.Layer):
    def __init__(self, num_classes):
        super(ArcMarginProduct_v21, self).__init__()
...
def getefn():
    ...
    pretrained_model = EFNS[EFF_VER](weights='/workspace/mnt/storage/zhangjunkang/gldv2/efficientnet/efficientnet-b3_weights_tf_dim_ordering_tf_kernels_autoaugment_notop.h5', include_top=False ,input_shape=[*INPUT_IMAGE_SIZE, 3])
...
def ArcFaceResNet():
    ...
    target = ArcMarginProduct_v21(NUM_CLASSES)(x)
...
with strategy.scope():
    model = ArcFaceResNet()
    model.load_weights("/workspace/mnt/storage/zhangjunkang/gldv2/data/2080_M3_adacos_S1_31-2-2-2/weights.epoch11.loss0.8731.valid_loss0.8285.hdf5", by_name=True)
    learning_rate_fn = tf.keras.optimizers.schedules.PiecewiseConstantDecay(boundaries, values)
    optimizer = tf.keras.optimizers.SGD(learning_rate = learning_rate_fn, momentum=0.9,decay = 1e-5)
...
while True:
    ...
    print("lr:{}".format(optimizer._decayed_lr(tf.float32).numpy()))
