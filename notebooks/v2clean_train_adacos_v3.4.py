...
# 3-----------------------loss weight
train_16fold = pd.read_csv(DATA_ROOT_PATH + 'dataTrain'+ str(COUNT_LIMITED) +'_stage1.csv')
...
TRAIN_FILENAMES = []
TRAIN_LIST = list(train_16fold['images'])
random.shuffle(TRAIN_LIST)
for i, img_name in enumerate(TRAIN_LIST):
    TRAIN_FILENAMES.append(TRAIN_GCS_PATH + img_name + ".tfrec")

val_16fold = pd.read_csv(DATA_ROOT_PATH + 'dataVal'+ str(COUNT_LIMITED) +'_stage1.csv')
VALID_FILENAMES = []
...
boundaries = [STEPS_PER_TPU_CALL*7,STEPS_PER_TPU_CALL*10]
values = [LR*1., LR*0.5, LR*0.1]
