...
# 4-----------------------
TRAIN_GCS_PATH = DATA_ROOT_PATH + 'tfrecord_train/'
...
val_16fold = pd.read_csv(DATA_ROOT_PATH + 'dataVal_stage1.csv')
VALID_FILENAMES = []
...
boundaries = [STEPS_PER_TPU_CALL*COUNT_STEP,STEPS_PER_TPU_CALL*COUNT_STEP*2]
values = [1*LR, 0.5*LR, 0.1*LR]
with strategy.scope():
    model = ArcFaceResNet()
...
print("Training steps per epoch:", STEPS_PER_TPU_CALL, "in increments of", BATCH_SIZE)
START_EPOCH = 0
epoch = START_EPOCH
