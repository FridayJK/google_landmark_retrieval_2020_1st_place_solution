...
model.load_weights('/workspace/mnt/storage/zhangjunkang/gldv2/data/'+ MODEL_VERSION_DIR +'/'+ MODEL_VERSION_HD5 +'.hdf5',by_name=True)
model.summary()
...
if __name__ == '__main__':
    ...
    feat_Path = "./result/feat/"+MODEL_VERSION_DIR
    os.makedirs(feat_Path,exist_ok=True)

    test_feat = extract_feat("test", images) ###
    np.save(feat_Path+"/testFeat_"+ MODEL_VERSION_HD5 +"_rmIgnored.npy",test_feat)

    index_feat = extract_feat("index")       ###
    np.save(feat_Path+"/indexFeat_"+ MODEL_VERSION_HD5 +".npy",index_feat)
    ...
    pred_Path = "./result/prediction/"+MODEL_VERSION_DIR
    os.makedirs(pred_Path,exist_ok=True)

    predictions = pd.DataFrame({'id':images,'images':solution_image_id,'Usage':Usage})
    predictions.to_csv(pred_Path+"/predictions_"+MODEL_VERSION_HD5+".csv",sep=',', index=False)